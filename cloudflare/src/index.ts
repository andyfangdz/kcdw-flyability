...
type Pointer = { key: string; run_id: string; assessed_at: string };
...
async function publish(request: Request, env: Env): Promise<Response> {
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    ...
    const pointer: Pointer = { key, run_id: report.run_id, assessed_at: report.assessed_at };
...
async function serve(request: Request, env: Env): Promise<Response> {
  ...
  if (url.pathname === '/health.json') {
    const age = Math.max(0, Math.floor((Date.now() - Date.parse(report.assessed_at)) / 1000));
    const stale = age > report.health.stale_after;
    return json({ ...report.health, run_id: report.run_id, age_seconds: age, stale, status: stale ? 'stale' : 'ok' });
  }
