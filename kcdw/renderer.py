...
def render(snapshot: dict, analysis: dict, now: datetime | None = None) -> tuple[str, dict]:
    ...
    for date in snapshot["report_dates"]:
        ...
        cells = "".join(f'<li class="score {score_class(w["score"])}"><span class="window">{esc(w["window"])}</span><strong>{w["score"]}%</strong><span>{esc(w["label"])}</span><small>{esc(w["reason"])}</small></li>' for w in day["windows"])
        hazards = "".join(f"<li>{esc(h)}</li>" for h in day["hazards"]) or "<li>None singled out</li>"
        cards.append(f'''<article class="day-card" id="d-{date}"><header><div><p class="eyebrow">{esc(dt.strftime("%A"))}</p><h2>{esc(dt.strftime("%b %-d"))}</h2></div><span class="confidence">{esc(day["confidence"].upper())} confidence</span></header><ul class="scores" aria-label="Two-hour flyability scores">{cells}</ul><p>{esc(day["narrative"])}</p><ul class="hazards">{hazards}</ul><p class="sunset">{caveat}</p></article>''')
