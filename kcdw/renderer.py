...
from .common import UTC, atomic_write, load_json, parse_time
...
def source_rows(snapshot: dict) -> str:
    ...
    for key, source in snapshot["sources"].items():
        ...
        if key == "radar_mosaic" and source["ok"]:
            ...
            provider = packet.get("forecast", {}).get("source", "WeatherNext 3")
            detail = f'Actual run {data.get("actual_run_utc", "unknown")}; fetched {data.get("fetched_at", "unknown")}; {provider} mean/p10/p90; fallback {bool(data.get("fallback"))}; cloud available; no ceiling/visibility/gust fields'
...
def render(snapshot: dict, analysis: dict, now: datetime | None = None, previous: dict | None = None) -> tuple[str, dict]:
    ...
    for date in snapshot["report_dates"]:
        ...
        for window in sorted(day["windows"], key=lambda w: w["window"]) if detailed else []:
            ...
            cell_items.append(f'<li class="score {score_class(window["score"])} {state}" data-window-state="{state}" data-date="{esc(date)}" data-window="{esc(window["window"])}"><span class="window">{esc(window["window"])} {state_label}</span><strong>{esc(score_label(window["score"]))}</strong></li>')
            window_reasons.append(f'<li><strong>{esc(window["window"])}</strong> · {esc(window["reason"])}</li>')
        window_detail = ('<details class="day-reasons"><summary>Window details &amp; confidence</summary><ul>'
                         + ''.join(window_reasons) + '</ul><p>' + esc(day['confidence_reason']) + '</p></details>'
                         if detailed else '<details class="day-reasons"><summary>Why this confidence?</summary><p>'
                         + esc(day['confidence_reason']) + '</p></details>')
        if 'confidence_score' in day:
            indices = (f'<p class="muted">Flyability index: {day["score"]}/95 · '
                       f'Weather confidence index: {day["confidence_score"]:g}/100. '
                       'Ordinal planning indices, not probabilities.</p>')
            window_detail = window_detail.replace('</details>', indices + '</details>')
        ...
        cards.append(f'''<article class="day-card" id="d-{date}" data-date="{date}" data-weekday="{esc(weekday)}"><header><div><p class="eyebrow">{esc(day_label)}</p><h2>{esc(dt.strftime("%b %-d"))}</h2><p class="day-horizon">{horizon}</p></div><span class="confidence">{esc(day["confidence"])} confidence</span></header><div class="day-body">{cells}<p class="narrative">{esc(day["narrative"])}</p>{window_detail}<section class="day-detail" aria-label="Hazards and daylight"><h3 class="field-label">Hazards &amp; daylight</h3><ul class="hazards">{hazards}</ul><p class="sunset">{caveat}</p></section></div></article>''')
    ...
    change_html = f'<section id="assessment-changes"><p class="section-number">02 / Assessment changes</p><h2>What changed?</h2><p class="muted">{esc(change_data["message"])}</p><ul class="changes-list">{change_items}</ul></section>'
    ...
    css = (Path(__file__).parent / "assets/fonts.css").read_text() + (Path(__file__).parent / "report.css").read_text() + (Path(__file__).parent / "context.css").read_text()
    freshness_script = (Path(__file__).parent / "freshness.js").read_text(encoding="utf-8")
    try:
        from .weekly_guidance import render_weekly, chart_css
        weekly_html = render_weekly(snapshot, now)
        css += chart_css()
    except Exception:
        weekly_html = '<section id="weekly-ensembles"><h2>Weekly model charts unavailable</h2><p>Supplementary chart data could not be validated. The daily outlook and official guidance remain independent; missing data is not favorable weather.</p></section>'
    forecast_script = (Path(__file__).parent / "assets/forecast-navigation.js").read_text(encoding="utf-8")
    script_policy = "script-src " + " ".join("'sha256-" + base64.b64encode(hashlib.sha256(script.encode()).digest()).decode() + "'" for script in (freshness_script, forecast_script)) + "; object-src 'none'; base-uri 'none'"
    section_nav = '<nav class="outlook-sections" aria-label="Outlook sections"><a href="#daily-outlook">Daily windows</a><a href="#assessment-changes">What changed</a><a href="#weekly-ensembles">Model comparison</a><a href="#regional-guidance">Regional context</a></nav>'
    ...
    guidance_html = f'<section class="model-policy" aria-label="Model guidance preference"><p class="eyebrow">Beyond 48 hours / Model guidance</p><h2>{esc(preferred)}</h2><p>AIFS-ENS supplies the independent comparison and fallback. The event page includes a separate WN2 member wind/rain check. Official NWS forecasts, AWC products and radar retain priority. WN3 cloud fraction is not a ceiling; missing ceiling, visibility and gust fields are not evidence of favorable conditions.</p></section>'
    ...
    context_start, context_end = report_window(snapshot, generated)
    context_html = ('<details id="regional-guidance" class="report-detail"><summary>Regional outlooks &amp; tropical context</summary>'
                    + render_context(snapshot.get("synoptic_context", {}), context_start, context_end, now) + '</details>')
    guidance_html = '<details class="source-section"><summary>Model guidance</summary>' + guidance_html + '</details>'
    legend = ''.join(f'<span class="{tone}">{label}</span>' for _, tone, label in BANDS)
    doc = f'''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta http-equiv="Content-Security-Policy" content="{esc(script_policy)}"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="color-scheme" content="light"><meta name="description" content="A seven-day weather outlook for local VFR flying at Essex County Airport, KCDW. Compare planning windows, hazards and forecast confidence."><title>KCDW Flyability · 7-Day Outlook</title><style>{css}</style></head><body data-design="field-notes" data-assessed-at="{esc(snapshot['collected_at'])}" data-stale-after="{stale_after}"><a class="skip-link" href="#main">Skip to forecast</a><header class="top"><div class="wrap"><a class="brand" href="/" aria-label="KCDW Flyability home"><span class="brand-symbol" aria-hidden="true"></span><span class="brand-name">KCDW<small>Flyability / Field notes</small></span></a><nav class="top-links" aria-label="Main navigation"><a class="nav-current" href="/" aria-current="page">Current outlook</a>{event_nav_links(now)}<a href="/history">History ↗</a><div class="updated"><span class="update-label">Assessment issued</span><time datetime="{esc(snapshot['collected_at'])}">{esc(generated.astimezone(TZ).strftime('%b %-d · %-I:%M %p %Z'))}</time>{badge}</div></nav></div></header><main class="wrap" id="main"><section class="hero"><div class="hero-intro"><p class="eyebrow">Essex County Airport / Caldwell, NJ</p><h1>Your flying outlook.</h1><p class="hero-summary">{esc(analysis['summary'])}</p><div class="mission"><span>Local VFR pattern work</span><span>2-hour sessions</span><span>All times Eastern</span></div></div><aside class="watch"><p class="eyebrow">The briefing</p><h2>Watch items</h2><ol class="hazards">{hazards}</ol><p class="watch-foot">Planning prospects, not flight clearance.<br>Check current conditions before departure.</p></aside></section>{guidance_html}<section class="picks" aria-label="Recommended planning days">{''.join(picks)}</section><section class="week-overview" aria-label="Seven-day overview"><div class="section-head"><div><p class="section-number">01 / Your flying week</p><h2>This week</h2></div><p class="muted">Select a day for the details ↓</p></div><nav class="week-strip" aria-label="Jump to a forecast day">{''.join(week)}</nav><div class="legend" role="group" aria-label="Score legend">{legend}</div></section>{section_nav}<section id="daily-outlook" class="days" aria-label="Daily forecasts">{''.join(cards[:3])}<div class="outlook-intro"><h2>Further out.</h2><p>Days 4–7 / Broad scenarios; timing is less certain.</p></div><div class="broad-grid">{''.join(cards[3:])}</div></section><div class="support-grid">{change_html}<section><p class="section-number">03 / From the field</p><h2>Current KCDW observation</h2><p class="muted">AWC METAR. Observations inform the near-term outlook.</p><p class="obs">{esc(obs)}</p></section></div>{weekly_html}{context_html}<details class="source-section"><summary>Source status and freshness</summary><div class="table-wrap"><table><thead><tr><th>Source</th><th>Status</th><th>Issued / fetched</th></tr></thead><tbody>{source_rows(snapshot)}</tbody></table></div></details><details class="source-section"><summary>How to read this outlook</summary><p>Categories describe planning prospects, not calibrated probabilities. The first three days show two-hour weather windows; later days describe broad scenarios. Confidence expresses uncertainty in the forecast, not pilot or aircraft capability.</p><p>NWS and AWC products guide the assessment. KTEB and KEWR TAFs are local proxies; KCDW has no routine TAF. WeatherNext 3 supplies preferred longer-range model guidance when fresh; AIFS-ENS is the independent comparison/fallback with independent WeatherNext 2 member diagnostics on the event page. Source gaps and model disagreement can reduce confidence.</p><p>Generated from a bounded weather snapshot, validated before publication. <a href="https://open-meteo.com/">Weather data by Open-Meteo.com</a> (CC BY 4.0); ECMWF AIFS-ENS via Open-Meteo. <a href="https://developers.google.com/weathernext/guides/bigquery">Google WeatherNext 3 via BigQuery</a>.</p></details><section class="disclaimer"><h2>Plan here.<br>Brief before you fly.</h2><p>This planning aid is not an official weather briefing, dispatch recommendation, or go/no-go decision. Assessments address meteorological comfort and do not account for pilot, aircraft, runway, NOTAM, daylight, traffic, or operational limits. Obtain a current official briefing and apply personal minimums before flight.</p></section><footer class="site-footer"><span>KCDW / Essex County Airport</span><span>Weather changes. Keep checking.</span></footer></main><noscript><p class="wrap">Live freshness requires JavaScript. Check the assessment timestamp before using this report.</p></noscript><script>{freshness_script}</script><script data-forecast-script>{forecast_script}</script></body></html>'''
